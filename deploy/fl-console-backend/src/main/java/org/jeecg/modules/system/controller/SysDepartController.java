...
//			}
    ...
    @RequestMapping(value = "/queryTreeListByPartyId", method = RequestMethod.GET)
    public Result<List<SysDepartTreeModel>> queryTreeListByPartyId(@RequestParam(name = "partyId", required = true) String partyId) {
        Result<List<SysDepartTreeModel>> result = new Result<>();
        try {
            FlPartyInfo flPartyInfo = flPartyInfoService.getById(partyId);
            List<SysDepartTreeModel> list = sysDepartService.queryDeptTreeListByOrgCode(flPartyInfo.getInfoc());
            result.setResult(list);
            result.setSuccess(true);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
