...
public class SysPermissionController {
	 ...
	 * @return
	 */
	@ApiOperation(value="获取全部菜单数据", notes="获取全部菜单数据")
	 ...
	 * @return
	 */
	@ApiOperation(value="获取一级菜单数据", notes="获取一级菜单数据")
	 ...
	 * @return
	 */
	@ApiOperation(value="查询子菜单数据", notes="查询子菜单数据")
	 ...
	 */
	@ApiOperation(value="批量根据父ID查询子级菜单", notes="批量根据父ID查询子级菜单")
...
//	}
	 ...
	 * @return
	 */
	@ApiOperation(value="查询当前用户拥有的菜单权限和按钮权限", notes="当前用户的菜单权限和按钮权限")
	 ...
	 * @return
	 */
	@ApiOperation(value="查询联邦方用户拥有的菜单权限和按钮权限", notes="当前用户的菜单权限和按钮权限")
	@RequestMapping(value = "/getFlUserPermission", method = RequestMethod.GET)
	public Result<?> getFlUserPermission(HttpServletRequest req) {
		...
		try {
			...
			log.info("req.getHeader(\"X-Access-Token\")==============>"+req.getHeader("X-Access-Token"));
			log.info("req.getHeader(\"tenant_id\")==========================>"+req.getHeader("tenant_id"));
			log.info("req.getHeader(\"Referer\")==========================>"+req.getHeader("Referer"));
			log.info("req.getHeader(\"tenant-id\")==========================>"+req.getHeader("tenant-id"));

			log.info("loginUser==========================>"+loginUser);
			List<SysPermission> metaList = sysPermissionService.queryByFlUser(loginUser.getUsername(), Integer.parseInt(tenantId));
