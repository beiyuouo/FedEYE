...
public class CommonController {
     ...
     * @return
     */
    @ApiOperation(value = "文件上传", notes = "文件上传，success为true时，message为保存的文件路径")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "biz", value = "文件目录，用户user，联邦方party，任务job", required = true, dataType = "string", paramType = "query"),
    ...
    @DynamicResponseParameters(properties = {
            ...
            @DynamicParameter(name = "message", value = "返回文件存储路径", required = true, dataTypeClass = String.class),
    ...
    public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
        ...
        File file1 = this.transferToFile(file);
        try {
            String type = FileTypeUtil.getType(file1);
            log.info("文件类型type是" + type);
    ...
    @ApiOperation(value = "预览图片", notes = "预览minio图片")
    @GetMapping(value = "/viewpic")
    public void viewpic(@RequestParam @ApiParam(name = "path", value = "文件路径", required = true) String path, HttpServletResponse response) {
        if (StringUtils.isEmpty(path)) {
            return;
        }
        String bucketName = MinioUtil.getBucketName();
        String fileName = path.substring(path.indexOf(bucketName) + bucketName.length() + 1);
        InputStream inputStream = MinioUtil.getMinioFile(bucketName, fileName);
        InputStream inputStream1 = MinioUtil.getMinioFile(bucketName, fileName);
        String type = FileTypeUtil.getType(inputStream1);
        Set<String> typeSet = new HashSet<>();
        typeSet.add("jpg");
        typeSet.add("png");
        typeSet.add("gif");
