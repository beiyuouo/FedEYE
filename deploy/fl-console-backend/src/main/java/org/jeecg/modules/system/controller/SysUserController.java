...
import org.jeecg.modules.system.util.AesEncode;
import org.jeecg.modules.system.util.PwdCheckUtil;
import org.jeecg.modules.system.util.TotpUtils;
...
public class SysUserController {
    ...
    public Result<SysUser> updateOrg(@RequestBody JSONObject jsonObject) {
        Result<SysUser> result = new Result<SysUser>();
        try {
            SysUser sysUser = sysUserService.getById(jsonObject.getString("id"));
            if (sysUser == null) {
                ...
                sysUserService.editUserWithDepart(user, orgCode);
                result.success("操作成功!");
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
     ...
     * @return
     */
    @ApiOperation(value = "当前用户的角色及状态", notes = "当前用户的角色及状态")
    @ApiImplicitParam(paramType = "header", name = "tenant_id", value = "联邦方id", dataType = "String", required = true)
    @DynamicResponseParameters(properties = {
            @DynamicParameter(name = "code", value = "代码", required = true, dataTypeClass = Integer.class),
            @DynamicParameter(name = "message", value = "消息", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "result", value = "[{'roleId':'','roleCode':'','status':''}]", required = true, dataTypeClass = List.class),
    ...
    public Result<?> showUserRole(HttpServletRequest req) {
        ...
        try {
            ...
            List<SysUserRole> userRoleList = sysUserRoleService.list(new QueryWrapper<SysUserRole>().lambda()
                    ...
                    .eq(SysUserRole::getPartyId, tenantId)
            );

            List<SysRole> flRoleList = sysRoleService.list();
            if (userRoleList == null || userRoleList.size() <= 0) {
                return Result.error("未找到用户相关角色信息");
            } else {
                HashMap<String, String> roleMap = new HashMap();
                for (SysRole flRol : flRoleList) {
                    roleMap.put(flRol.getId(), flRol.getRoleCode());
                }
     ...
     * @return
     */
    @ApiOperation(value = "校验用户账号是否唯一", notes = "校验用户账号是否唯一，需要检验什么就传什么")
    @RequestMapping(value = "/checkOnlyUser", method = RequestMethod.GET)

    @DynamicResponseParameters(properties = {
            @DynamicParameter(name = "code", value = "代码", required = true, dataTypeClass = Integer.class),
            @DynamicParameter(name = "message", value = "消息", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "result", value = "false：程序发生异常", required = true, dataTypeClass = Boolean.class),
            @DynamicParameter(name = "success", value = "true：唯一", required = true, dataTypeClass = Boolean.class),
            @DynamicParameter(name = "timestamp", value = "时间戳", required = true, dataTypeClass = long.class)
    })
    ...
    public Result<?> changePassword(@RequestBody SysUser sysUser) {
        SysUser u = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getUsername, sysUser.getUsername()).or().eq(SysUser::getPhone, sysUser.getPhone()));
        if (u == null) {
...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    @ApiOperationSupport(params = @DynamicParameters(name = "jsonObject", properties = {
            @DynamicParameter(name = "phone", value = "手机号", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "smscode", value = "验证码", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "username", value = "用户名", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "usertype", value = "用户类型，1管理平台用户，2联邦平台用户", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "password", value = "密码", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "email", value = "邮箱", required = false, dataTypeClass = String.class)
    }))
    ...
    public Result<JSONObject> userRegister(@RequestBody JSONObject jsonObject) {
        ...
        String secretKey = String.valueOf(redisUtil.get(phone));
        ...
        if (oConvertUtils.isEmpty(password)) {
            result.error500("请设置密码");
            return result;
        }
        Preconditions.checkNotNull(password);
        if (!PwdCheckUtil.checkPasswordLength(password, "8", null)
            ...
            result.error500("密码必须由至少8位数字、大小写字母和特殊符号组成!");
            return result;
        }
        ...
        if (!TotpUtils.verify(secretKey, smscode)) {
            result.error500("手机验证码错误");
            return result;
        }

        try {
            String secret = redisUtil.get(phone).toString();
            ...
            if (usertype.equals("1")) {
                String roles = "ee8626f80f7c2619917b6236f3a7f02b";
                sysUserService.addUserWithRole(user, roles);
            } else {
                String roles = "1417211331594141698";
                sysUserService.addUserWithFlRole(user, roles);
            }

            // 注册成功后返回token
            // 生成token
            String token = AesEncode.encrypt(JwtUtil.sign(username, password));
            // 设置token缓存有效时间
    ...
    public Result<JSONObject> userRegister1(@RequestBody JSONObject jsonObject) {
        SysUser user = new SysUser();
        Result<JSONObject> result = new Result<JSONObject>();
        String username = jsonObject.getString("username");
        // 1管理平台用户，2联邦平台用户
    ...
    @ApiOperation(value = "用户手机号验证", notes = "用户手机号验证")
    @ApiOperationSupport(params = @DynamicParameters(name = "jsonObject", properties = {
            @DynamicParameter(name = "phone", value = "手机号", required = true, dataTypeClass = String.class),
            @DynamicParameter(name = "smscode", value = "验证码", required = true, dataTypeClass = String.class)
    }))
    ...
    public Result<SysUser> passwordChange(@RequestParam(name = "password") String password,
        ...
        sysUser = this.sysUserService.getOne(new LambdaQueryWrapper<SysUser>().eq(SysUser::getPhone, phone));
        if (sysUser == null) {
